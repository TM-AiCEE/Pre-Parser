...
namespace ParseUserLog
{
    ...
    static class Program
    {
        ...
        static void Traverse(this IDictionary<DateTime, Record> records, IDictionary<int, List<Stage>> stages)
        {
                                 ...
                                 select pair)
            {
                ...
                foreach (var player in data.players)
                {
                    ...
                    var stage = stages[data.table.board.Length].FindSameStage(cards, board, player.Action);
                    if (stage == null)
                    {
        ...
        public class Data
        {
            ...
            {
                table?.Canonicalize();
                return players.All(player => player.Canonicalize());
            }
        }
        ...
        public class Player
        {
            ...
            public string Action => folded ? "fold"
                                    : allIn ? "allIn"
                                    : bet > 0 ? "bet"
                                    : "nothing"; // ???
